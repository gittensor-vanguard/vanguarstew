...
def check_score_floor(result: dict, fail_under: float | None) -> str | None:
    ...
    ``0.0``) or a zero-task single-repo run (``tasks == 0`` with the same placeholder) has no real
    score to gate, so it is skipped (returns ``None``) instead of being reported as "0.000 below
    threshold". A genuinely scored run whose composite is really ``0.0`` (``scored_repos > 0``, or a
    single-repo run with ``tasks > 0``) is still gated normally.
    """
