"""CLI: print dual-order judge agree rate from a replay artifact.

  python -m scripts.order_agree_rate result.json
...
from benchmark.order_agree_rate import order_agree_rate_headline, summarize_order_agree_rate
...
def run(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Report dual-order judge agree rate from stats")
    ...
    try:
        ...
        return int(exc.code)
    summary = summarize_order_agree_rate(artifact)
    print(order_agree_rate_headline(summary), file=sys.stderr)
