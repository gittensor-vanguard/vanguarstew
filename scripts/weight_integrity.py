"""CLI: gate whether a replay artifact's blend weights are present and valid.
  ...
  python -m scripts.weight_integrity artifact.json
  python -m scripts.weight_integrity artifact.json --strict

With --strict the process exits non-zero when the weight integrity gate fails.
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Gate a replay artifact on blend-weight integrity",
    )
