"""CLI: gate whether a run judged enough tasks for its headline to be trustworthy.
...
non-zero when the run judged fewer than ``--min-tasks`` tasks.
"""
...
def main() -> None:
    ...
    ap.add_argument("--strict", action="store_true",
                    help="exit 1 when the sample is too small (for CI gating)")
    args = ap.parse_args()
