...
def load_artifact(path: str) -> dict:
    ...
    message rather than a raw traceback / errno string:

    - a **broken symlink** (dangling target): the link exists but its target is gone, so
      ``open`` raises ``FileNotFoundError`` — reported as a broken link, not "not found";
    - a **symlink loop**: ``open`` raises ``OSError(ELOOP)``, named explicitly instead of
      leaking a raw ``[Errno 40] ...`` string;
    - ``FileNotFoundError``: the path does not exist.
    - ``PermissionError``: the file exists but is not readable.
    - ``IsADirectoryError``: the path is a directory, not a file.
    - any other ``OSError``: the raw ``str(exc)`` is echoed.
