...
### Check-row verdict helper (`_is_passed`)

The verdict helper is documented on its own because it is the deliberate **inverse** of
`_is_number` on numpy types:

- WHEN `value` is a `bool` (including a `bool` subclass) THEN `_is_passed(value)` SHALL be true.
- WHEN `value` is a `numpy` scalar bool THEN `_is_passed(value)` SHALL be true; the module
  recognizes it by `type(value).__name__ in ("bool_", "bool8", "bool")` (numpy 1.x uses
  `bool_`/`bool8`, numpy 2.x uses `bool`), so no numpy import is required.
- WHEN `value` is an `int` `0` or `1` THEN `_is_passed(value)` SHALL be **false** — a check
  verdict must be a genuine boolean, not a truthy integer.
- **Asymmetry (deliberate):** `_is_passed` *accepts* a numpy scalar bool, while `_is_number`
  (above) *rejects* every numpy scalar. A weight must be an exact Python number the whole blend
  can trust; a check row's `passed` verdict may legitimately arrive as a numpy bool from an
  upstream vectorized comparison.
