...
3. **As a reviewer**, the handling of unscored / non-finite / malformed points, the placeholder
   `scored_repos: 0` `0.0`, the exact **rounding mode**, and the delta / regression / first-last
   semantics — including a series scored only at its endpoints — are written down.
...
#### Rounding mode (as-built, explicit)

The returned score SHALL be `round(float(composite_mean), 3)`. The rounding mode is that of the
Python built-in [`round`](https://docs.python.org/3/library/functions.html#round): **round-half-to-even
(banker's rounding)**, applied to the float value.

- On an **exact** binary half-tie the result resolves to the **even** last digit — e.g.
  `round(0.5, 0) == 0.0`, `round(1.5, 0) == 2.0`, `round(2.5, 0) == 2.0` (not `3.0`),
  `round(3.5, 0) == 4.0`. This is the defining property of the mode.
- At the module's **3-decimal** precision, a decimal literal ending in `…5` at the 4th place is
  **almost never** an exact binary tie: most decimal fractions cannot be represented exactly as a
  float, so the stored value lies just above or just below the mathematical midpoint and rounds to
  whichever 3-decimal neighbour it is actually nearer — the even-digit rule only bites on a true
  binary tie. As observed on the as-built module: `0.0005 → 0.001`, `0.0015 → 0.002`,
  `0.6125 → 0.613`, `0.6135 → 0.614`, `0.1235 → 0.123`, `0.1245 → 0.124`, `0.2415 → 0.241`, and
  the negative tie `-0.0005 → -0.001`, `-0.1245 → -0.124`.
- Either way the value is **exactly** `round(float(composite_mean), 3)` — deterministic and
  reproducible across runs and platforms (IEEE-754 double). There is no ambiguity: the tie
  behavior is fully pinned by the tests below, not left to interpretation.

The **same rounding** governs `_round` (the per-point value, the `delta`, and `change`) and the
regression `drop` (`round(from - to, 3)`).
...
- `first` / `last` SHALL be the first and last **scored** values (skipping any leading/trailing
  ...
  across **all** scored values (a mid-series spike or dip is captured, independent of the endpoints).
  WHEN no point is scored THEN all five SHALL be `None`.
...
#### Endpoint-only-scored series (bridging, explicit)

- WHEN the series is scored **only at its endpoints** and every intermediate point is unscored — an
  `[scored, unscored, …, unscored, scored]` shape (e.g. error artifacts between two real runs) —
  THEN the delta and regression math SHALL bridge the unscored middle as a single step between the
  two scored endpoints:
  - the last point's `delta` SHALL be the change from the **first** scored point (`last − first`);
  - a drop beyond the threshold SHALL be reported **once** as a single `{from_label, to_label, drop}`
    naming the two **scored endpoints** — the first scored label as `from_label` and the last scored
    label as `to_label` — **not** the unscored neighbours;
  - `scored` SHALL be `2` while `total` counts every surviving (including unscored) point;
  - a rising endpoint pair (`last > first`) SHALL flag **no** regression.
- The degenerate 2-point endpoint shapes — `[scored, scored]`, `[scored, unscored]`, and
  `[unscored, scored]` — SHALL follow the same rules: `[scored, scored]` yields a direct
  `from → to` regression when it drops beyond the threshold; `[scored, unscored]` and
  `[unscored, scored]` each carry `scored == 1`, set `first == last` with `change == 0.0`, and flag
  no regression.
...
- `trend()` SHALL **NOT mutate** its `series` argument or any artifact within it, for well-formed
  **and** every degenerate shape (non-`list` series, malformed entries, unscored / non-finite /
  placeholder artifacts, endpoint-only-scored series). The contract is verified by deep-copying each
  input before the call and asserting the input is unchanged afterward (a value-equality, not a
...
- `tests/test_spec_012_trend_headline.py` exercises each EARS block above, including the
  ...
  the **rounding mode** (banker's rounding, with pinned tie and negative-tie outputs), series/entry
  coercion, the delta/regression/first-last summary semantics, the **endpoint-only-scored**
  regression path (and its 2-point degenerate variants), the headline branches, and the deep
  non-mutation / no-I/O purity checks. Every assertion is pinned against the module's live output.
