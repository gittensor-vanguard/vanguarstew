...
- `action` SHALL be mapped onto `VALID_ACTIONS` (`merge`, `request-changes`, `reject`, `triage`,
  `assign-reviewer`, `release`, `plan`, `patch`, `close`, `label`) via a case-insensitive exact
  match or a known synonym (e.g. `assign_reviewer`/`assign reviewer` → `assign-reviewer`).
  - A **non-string** `action` SHALL default to `plan` **and log a warning** naming the offending
    type/value.
  - An **unrecognized string** `action` SHALL default to `plan` **without logging** (a merely
    out-of-vocabulary string is expected model noise, not an anomaly worth a warning).
  - Neither case SHALL raise.
