...
`decisive_rate` derives tie share from replay tally; `order_agree_rate` covers dual-order tasks
only. `summarize_tie_order_share()` reports `tie / total` categorized outcomes from
`judge_order_stats` for judge-stability dashboards.

## User stories

1. **As a benchmark operator**, I can read tie outcome share before trusting judge stability
   metrics.
...
- WHEN every count is a non-negative `_is_int` THEN `total` SHALL be their sum and `tie` SHALL be
  the tie count.
- WHEN any count is invalid THEN the slice SHALL return
  `{"total": None, "tie": None, "tie_order_share": None}`.
- WHEN all counts are valid and `total > 0` THEN `tie_order_share` SHALL be `round(tie / total, 3)`.
- WHEN all counts are valid and `total == 0` THEN `total` SHALL be `0`, `tie` SHALL echo the tie
  count, and `tie_order_share` SHALL be `None`.
...
- `tests/test_spec_041_tie_order_share.py` exercises each EARS block above.
