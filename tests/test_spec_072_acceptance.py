...
# --- Numeric helper (each edge asserted independently) -------------------------------------------

def test_is_number_accepts_finite_int_and_float():
...
def test_is_number_rejects_bool():
    # isinstance(True, int) is True in Python, but a flag is not a count.
    assert _is_number(True) is False
    assert _is_number(False) is False


def test_is_number_rejects_nan():
    assert _is_number(math.nan) is False
    assert _is_number(float("nan")) is False


def test_is_number_rejects_inf_and_negative_inf():
    assert _is_number(math.inf) is False
    assert _is_number(float("inf")) is False
    assert _is_number(-math.inf) is False


def test_is_number_rejects_str_and_none():
    assert _is_number("2") is False
    assert _is_number(None) is False


def test_is_number_rejects_oversized_int():
    # 10 ** 400 is an exact, platform-independent int beyond float range; float() raises
    # OverflowError, which is caught and yields False rather than propagating.
...
def test_partition_error_top_level_string():
    assert _partition_error({"error": "boom"}) == "boom"


def test_partition_error_top_level_non_string_truthy():
    # A truthy error of any type is returned unchanged (the value, not a bool / not stringified).
    assert _partition_error({"error": {"code": 1}}) == {"code": 1}
    assert _partition_error({"error": 5}) == 5


def test_partition_error_falsy_is_not_a_failure():
    for falsy in ("", 0, False, None, [], {}):
        assert _partition_error({"error": falsy}) is None


def test_partition_error_per_repo_dict_row():
    assert _partition_error({"per_repo": [{"tasks": 3}, {"error": "clone failed", "tasks": 0}]}) == (
        "clone failed")
    # a non-string truthy per-repo error is returned unchanged too
    assert _partition_error({"per_repo": [{"error": {"code": 2}}]}) == {"code": 2}
...
def test_check_rows_list_warns_on_non_list_checks(caplog):
    ...
    assert any(
        r.message == "acceptance: checks is int, not a list; treating as empty"
        for r in caplog.records
    )
    # a None checks key is silent
    caplog.clear()
    with caplog.at_level(logging.WARNING, logger="benchmark.acceptance"):
...
def test_check_rows_list_warns_when_all_unusable(caplog):
    with caplog.at_level(logging.WARNING, logger="benchmark.acceptance"):
        assert _check_rows_list([{"name": "a"}]) == []
