"""Tests for disagree share summary and CLI (deterministic, offline)."""
...
def test_slice_summary_disagree_share():
    out = _slice_summary(_stats(agree=2, disagree=2, tie=0, single=0, offline=0))
    assert out["total"] == 4
    assert out["disagree"] == 2
    assert out["disagree_share"] == 0.5


def test_zero_total_yields_none_share():
    out = _slice_summary(_stats(0, 0, 0, 0, 0))
    assert out["total"] == 0
    assert out["disagree_share"] is None


def test_malformed_stats_yield_none():
    art = {"judge_order_stats": {"agree": 1, "disagree": "many", "tie": 0, "single": 0, "offline": 0}}
    assert _slice_summary(art)["disagree_share"] is None


def test_negative_counts_rejected():
    assert _slice_summary(_stats(-1, 0, 0, 0, 0))["disagree_share"] is None


def test_single_artifact_reports_decimal_share():
    summary = summarize_disagree_share(_stats(agree=4, disagree=1, tie=0, single=0, offline=0))
    assert summary["kind"] == "single"
    assert summary["disagree_share"] == 0.2
    assert summary["partitions"] is None


def test_missing_stats_yields_none():
    summary = summarize_disagree_share({"composite_mean": 0.5})
...
def test_generalization_reports_partitions_and_overall():
    ...
    assert summary["disagree_share"] == round(1 / 9, 3)
    assert summary["partitions"]["tuned"]["disagree_share"] == 0.0
    assert summary["partitions"]["held_out"]["disagree_share"] == 0.2
...
def test_headline_formats_decimal_as_percentage():
    summary = summarize_disagree_share(_stats(agree=2, disagree=2, tie=0, single=0, offline=0))
    assert "50.0%" in disagree_share_headline(summary)
    ...
    assert disagree_share_headline("nope") == "disagree share: no judge stats available"
    assert "n/a" in disagree_share_headline({"total": 3, "disagree": 1, "disagree_share": None})


def test_headline_nan_share_does_not_crash():
    assert "n/a" in disagree_share_headline({
        "total": 3,
        "disagree": 1,
        "disagree_share": float("nan"),
    })
...
def test_cli_success(tmp_path, capsys):
    ...
    assert body["disagree_share"] == 0.2
...
def test_module_main_no_arg_exits_nonzero():
    proc = subprocess.run(
        [sys.executable, "-m", "scripts.disagree_share"],
