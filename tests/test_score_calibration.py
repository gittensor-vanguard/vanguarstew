...
from benchmark.score_corpus import CORPUS_DIR, MANIFEST_PATH, SCENARIOS_DIR  # noqa: E402

_VALID = {
    "id": "sample",
    "description": "sample scenario",
    "function": "module_recall",
    "inputs": {
        "plan": [{"title": "Fix agent loader", "files": ["agent/loader.py"]}],
        "revealed": [{"files": ["agent/loader.py"]}],
    },
    "expected": {"module_recall": 1.0, "matched_modules": ["agent"]},
}
...
def test_validate_scenario_rejects_unknown_function():
    bad = dict(_VALID, function="not-a-function")
    assert any("unknown function" in err for err in validate_scenario(bad))


def test_run_scenario_reports_pass_and_fail():
    passed = run_scenario(_VALID)
    assert passed["passed"] is True
    assert passed["function"] == "module_recall"
    failed = run_scenario(dict(_VALID, expected={"module_recall": 0.0}))
    assert failed["passed"] is False
    assert "module_recall" in failed["mismatches"]


def test_run_scenario_honors_float_tolerance():
    close = run_scenario(dict(_VALID, expected={"module_recall": 0.9999}, tolerance=0.01))
...
def test_bump_level_scenario_accepts_list_semvers():
    corpus = load_corpus()
    scenario = next(s for s in corpus if s["id"] == "bump-level-minor")
    result = run_scenario(scenario)
    assert result["passed"] is True


def test_released_version_tuple_matches_list_expected():
    corpus = load_corpus()
    scenario = next(s for s in corpus if s["id"] == "released-version-window")
    result = run_scenario(scenario)
    assert result["passed"] is True
...
def _run_cli(*args):
    return subprocess.run(
        [sys.executable, "-m", "scripts.calibrate_score", *args],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
...
def test_cli_passes_on_shipped_corpus():
    ...
    assert "score calibration: PASS" in proc.stderr
    assert '"scenario_count": 35' in proc.stdout
...
def test_cli_strict_fails_on_bad_corpus(tmp_path):
    ...
    (scenarios / "bad.json").write_text(json.dumps(dict(_VALID, id="bad", expected={"module_recall": 0.0})),
                                        encoding="utf-8")
    proc = _run_cli("--corpus-root", str(root), "--strict")
    assert proc.returncode == 1
    assert "score calibration: FAIL" in proc.stderr
