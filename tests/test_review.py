...
def test_pr_number_normalizes_non_scalar_and_bool():
    ...
    assert _pr_number({"number": 7.5}) is None
...
def test_review_pr_prompt_uses_pr_number_not_raw_number_field():
    ...
    for number in ("7", 7.0):
        llm = _CaptureUserLLM()
        review_pr({"number": number, "title": "Fix bug", "author": "a", "files": []}, None, llm)
        assert llm.last_user.startswith("PULL REQUEST #7: Fix bug")
...
def test_system_prompt_names_non_redundancy_as_high_rubric_axis():
    # REVIEW.md priority order: correctness & tests, scope fit, Non-redundancy (High), then
    # quality & clarity — the SYSTEM prompt must match that order and rank.
    assert "(1) correctness and tests" in SYSTEM.lower()
    assert "(2) scope fit" in SYSTEM.lower()
...
def test_review_pr_prompt_includes_non_redundancy_guidance():
    llm = _CaptureUserLLM()
    review_pr({"number": 1, "title": "Add metric", "author": "a", "files": ["benchmark/new_metric.py"]},
              None, llm)
