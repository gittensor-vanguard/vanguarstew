...
# --- load_artifact bad-path handling: a clean message + exit 2, never a raw errno (#1613) --------


def test_cli_directory_path_exits_two(tmp_path):
    # A directory artifact path raises IsADirectoryError on POSIX; the CLI must exit 2 with an
    # actionable message rather than crash with a raw traceback / bare errno.
    proc = subprocess.run(
        [sys.executable, "-m", "scripts.row_integrity", str(tmp_path)],
        cwd=ROOT, capture_output=True, text=True,
    )
    assert proc.returncode == 2
    assert "directory" in proc.stderr and "Traceback" not in proc.stderr
