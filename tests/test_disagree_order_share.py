...
def _stats(agree=2, disagree=1, tie=1, single=0, offline=0):
    return {
        "composite_mean": 0.6,
        "judge_order_stats": {
            "agree": agree,
...
def test_slice_summary_disagree_order_share():
    out = _slice_summary(_stats(agree=1, disagree=2, tie=0, single=1, offline=0))
    assert out["total"] == 4
    assert out["disagree"] == 2
    assert out["disagree_order_share"] == 0.5


def test_share_is_fraction_of_all_categorized_not_just_dual_order():
    # 1 disagree out of 5 categorized (agree+disagree+tie+single+offline) -> 0.2, even though the
    # dual-order disagreement rate (disagree/(agree+disagree+tie)) would be 1/3.
    out = _slice_summary(_stats(agree=1, disagree=1, tie=1, single=1, offline=1))
    assert out["total"] == 5
    assert out["disagree_order_share"] == 0.2
...
def test_malformed_stats_yield_none():
    art = {"judge_order_stats": {"agree": 0, "disagree": "many", "tie": 0, "single": 0, "offline": 0}}
...
def test_single_artifact_reports_decimal_share():
    summary = summarize_disagree_order_share(_stats(agree=3, disagree=1, tie=0, single=1, offline=0))
    assert summary["kind"] == "single"
    assert summary["disagree_order_share"] == 0.2
    assert summary["partitions"] is None


def test_multi_artifact_reads_top_level_stats():
    artifact = dict(_stats(agree=1, disagree=1, tie=0, single=0, offline=0))
    artifact["per_repo"] = [{"repo": "a"}, {"repo": "b"}]
    summary = summarize_disagree_order_share(artifact)
    assert summary["kind"] == "multi"
    assert summary["disagree_order_share"] == 0.5
    assert summary["partitions"] is None
...
def test_generalization_reports_partitions_and_overall():
    summary = summarize_disagree_order_share({
        ...
        "tuned": _stats(agree=2, disagree=2, tie=0, single=0, offline=0),
    ...
    assert summary["disagree_order_share"] == round(2 / 8, 3)
    assert summary["partitions"]["tuned"]["disagree_order_share"] == 0.5
    assert summary["partitions"]["held_out"]["disagree_order_share"] == 0.0


def test_generalization_missing_partitions():
    summary = summarize_disagree_order_share({
        "generalization_gap": 0.0,
        "tuned": {"judge_order_stats": {"agree": 0, "disagree": 1, "tie": 0, "single": 0, "offline": 0}},
        "held_out": {},
    })
...
def test_generalization_malformed_partition_does_not_crash():
    summary = summarize_disagree_order_share({
        ...
        "held_out": {"judge_order_stats": {"agree": 0, "disagree": None, "tie": 0, "single": 0, "offline": 0}},
    })
    assert summary["disagree_order_share"] is None
...
def test_headline_formats_decimal_as_percentage():
    summary = summarize_disagree_order_share(_stats(agree=1, disagree=1, tie=0, single=2, offline=0))
    assert "25.0%" in disagree_order_share_headline(summary)
...
def test_cli_success(tmp_path, capsys):
    path = _write(tmp_path, "ok.json", json.dumps(_stats(agree=3, disagree=1, tie=0, single=1, offline=0)))
...
def test_cli_generalization_reports_partitions(tmp_path, capsys):
    ...
    assert cli.run([path]) == 0
    body = json.loads(capsys.readouterr().out)
    assert body["partitions"]["tuned"]["disagree"] == 2
