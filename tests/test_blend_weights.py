...
def test_load_artifact_broken_symlink_reports_dangling_target(tmp_path, capsys):
    ...
    assert excinfo.value.code == 2
    err = capsys.readouterr().err
    assert f"artifact is a broken symlink (target does not exist): {link}" in err
...
def test_load_artifact_not_a_directory_error_is_handled(monkeypatch, tmp_path, capsys):
    path = str(tmp_path / "parent-is-a-file" / "run.json")
    ...
    assert excinfo.value.code == 2
    err = capsys.readouterr().err
    assert "artifact path is not a file (a parent component is not a directory)" in err
    assert "Traceback" not in err
