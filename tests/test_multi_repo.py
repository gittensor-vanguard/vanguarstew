...
@pytest.mark.skipif(shutil.which("git") is None, reason="git required")
def test_multi_repo_skips_unusable_repo():
    good = _tiny_repo(tempfile.mkdtemp())
    tiny = _tiny_repo(tempfile.mkdtemp(), n=2)  # too small for horizon -> no tasks
    try:
        res = run_multi_replay([good, tiny], agent_file=AGENT, n_tasks=2, horizon=5, seed=0)
        assert res["repos"] == 2
        assert res["scored_repos"] == 1 and res["skipped"] == 1
        # the skipped repo is still reported, with its error surfaced
        assert any("error" in r for r in res["per_repo"])
    finally:
