...
Two concerns are covered here, both of which feed benchmark scoring by file:

- Path-list parsing (#137): `revealed_window` must report the *exact* paths a commit
  touched — including filenames with spaces and shell-sensitive characters. These
  regressions pin the NUL-delimited (`git ... -z`) parsing and keep `revealed_window`
  behavior covered for both normal commits and first-parent merges.
...
def test_generate_tasks_preserves_large_revealed_file_lists():
    ...
    try:
        ...
        for i in range(n_files):
            _write(repo, f"mod{i}/file.py", f"x = {i}\n")
        _git(repo, "add", "-A")
