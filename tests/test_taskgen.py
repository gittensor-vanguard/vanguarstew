"""Tests for replay-task generation from git history — offline, git-backed.

`revealed_window`'s file-attribution ground truth is covered from angles that landed
independently and must all hold together:
...
- Full file-list retention (#157): task metadata must retain *every* changed path (no
  truncation), so a commit touching many modules is scored against all of them.

A single reusable history fixture (linear commits, a path with a space, and a non-fast-forward
first-parent merge) exercises `linear_history` ordering and the file-attribution properties
together, so a future change can't silently satisfy one and regress another.
"""
...
def test_revealed_window_preserves_paths_with_spaces_and_specials():
    ...
    try:
        ...
        shutil.rmtree(repo, ignore_errors=True)
# --- generate_tasks: full file-list retention (#157) -------------------------------
...
def test_generate_tasks_preserves_large_revealed_file_lists():
    ...
    try:
        ...
        for i in range(11):
            _commit(repo, f"seed{i}.txt", f"{i}\n", f"seed {i}")

        # One commit touching more than 20 top-level modules.
        n_files = 25
        for i in range(n_files):
            full = os.path.join(repo, f"mod{i}/file.py")
            os.makedirs(os.path.dirname(full), exist_ok=True)
            with open(full, "w", encoding="utf-8") as f:
                f.write(f"x = {i}\n")
        _run(repo, "add", "-A")
        _run(repo, "commit", "-q", "-m", "touch many modules")
