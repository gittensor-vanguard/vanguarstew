"""Tests for replay-task generation, focused on git path-list parsing.
...
@pytest.mark.skipif(shutil.which("git") is None, reason="git required")
def test_revealed_window_first_parent_merge_reports_merged_files():
    """A first-parent merge reports the files it brought onto the main line.

    `linear_history` walks `--first-parent`, so merge commits are legitimate
    revealed actions. Showing a merge against its first parent surfaces the files
    it introduced; the default combined diff would drop them, collapsing the
    structural ground truth on repos that land work through merges.
    """
    repo = tempfile.mkdtemp()
    try:
        _init(repo)
        ...
        # feat's file lands on main via the merge; the space in the path must
        # survive whole, exercising --first-parent and -z together.
        assert entry["files"] == ["feature file.txt"]
    finally:
        shutil.rmtree(repo, ignore_errors=True)
