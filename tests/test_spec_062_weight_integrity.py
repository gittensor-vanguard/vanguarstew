...
def test_numpy_bool_standin_matches_the_modules_type_check():
    # Prove the stand-in actually exercises the module's branch: weight_integrity._is_passed keys
    # off `type(value).__name__ in ("bool_", "bool8", "bool")`, so the stand-in's *instance* type
    # name must be one of those — not the class variable, not isinstance.
    assert type(_NumpyBool()).__name__ == "bool_"
...
def test_is_number_rejects_oversized_int_explicitly():
    # Restated at the _is_passed/_is_number boundary so the overflow branch is unmissable in
    # review: math.isfinite raises OverflowError on an int too large to convert to float.
    assert _is_number(10 ** 400) is False
    assert _is_number(-(10 ** 400)) is False
