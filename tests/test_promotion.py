...
# --- #1254: a per-repo clone/freeze failure must not be promoted on the strength of the ------
# repos that did succeed. `run_multi_replay` never aborts the batch for one bad repo: it records
# the failure inline as `per_repo[i] == {"error": ..., "tasks": 0}` and keeps going, so the
# artifact carries a perfectly numeric top-level composite_mean and no top-level `error` at all.


def test_a_per_repo_error_fails_run_completed_even_with_a_scored_composite():
    result = _result(composite=0.7, margin=2, disagreement=0.1)
    result["per_repo"] = [
        {"repo": "good-repo", "tasks": 3, "composite_mean": 0.7},
        {"repo": "bad-repo", "error": "clone failed", "tasks": 0},
    ]
    result = check_promotion(result)
    assert result["passed"] is False
    assert "run_completed" in failed_checks(result)


def test_a_clean_per_repo_batch_still_passes_run_completed():
    # Control: same shape, no per-repo error, so run_completed is unaffected by the new scan.
    result = _result(composite=0.7, margin=2, disagreement=0.1)
    result["per_repo"] = [
        {"repo": "a", "tasks": 3, "composite_mean": 0.7},
        {"repo": "b", "tasks": 2, "composite_mean": 0.6},
    ]
    result = check_promotion(result)
