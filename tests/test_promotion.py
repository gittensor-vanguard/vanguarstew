...
def test_unscored_multi_repo_placeholder_fails_run_completed():
    result = check_promotion({"composite_mean": 0.0, "scored_repos": 0, "repos": 2})
    assert result["passed"] is False
    assert "run_completed" in failed_checks(result)


def test_single_repo_zero_tasks_fails_run_completed():
    result = check_promotion({"composite_mean": 0.0, "tasks": 0})
    assert result["passed"] is False
    assert "run_completed" in failed_checks(result)
