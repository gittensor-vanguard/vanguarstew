...
def test_render_truncation_preserves_valid_json_and_all_judged_fields():
    # Overload every axis: structural abridgement must stay valid JSON under budget and keep
    # the three judged fields (possibly with an explicit _abridged note), never mid-slice.
    rendered = _render({
    ...
    assert len(rendered) <= _RENDER_BUDGET
    assert {"philosophy", "plan", "rationale"} <= set(decoded)
