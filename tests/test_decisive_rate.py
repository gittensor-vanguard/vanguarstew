...
# --- generalization: sum tuned/held_out partition tallies (mirrors win_rate) --------------------
...
def test_generalization_none_slice_returns_fresh_dicts():
    art = {"generalization_gap": 0.0,
           "tuned": {"tally": {"challenger": 1, "baseline": 0, "tie": 0}},
           "held_out": {}}
    first = summarize_decisive_rate(art)
    second = summarize_decisive_rate(art)
    assert first["partitions"]["held_out"] is not second["partitions"]["held_out"]
    first["partitions"]["held_out"]["total"] = 99
    assert second["partitions"]["held_out"]["total"] is None
