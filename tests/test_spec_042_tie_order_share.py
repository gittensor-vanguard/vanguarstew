...
def test_is_int_rejects_bool():
    ...
    assert _slice_summary(_stats(agree=0, disagree=0, tie=True, single=0, offline=0))[
        "tie_order_share"
    ] is None
...
def test_is_int_rejects_float_whole_numbers(value):
    ...
    art = {"judge_order_stats": {"agree": 0, "disagree": 0, "tie": value, "single": 0, "offline": 0}}
...
def test_slice_summary_happy_path():
    ...
    assert out == {"total": 4, "tie": 2, "tie_order_share": 0.5}
...
def test_slice_summary_malformed_stats():
    ...
    assert _slice_summary(art) == {"total": None, "tie": None, "tie_order_share": None}
...
def test_single_and_multi_kinds():
    single = summarize_tie_order_share(_stats(agree=4, disagree=0, tie=1, single=0, offline=0))
    assert single["kind"] == "single"
    assert single["tie_order_share"] == 0.2
    assert single["partitions"] is None

    multi = summarize_tie_order_share({
        "per_repo": [{}, {}],
        **_stats(agree=4, disagree=0, tie=1, single=0, offline=0),
    })
    assert multi["kind"] == "multi"
    assert multi["tie_order_share"] == 0.2
...
def test_summary_always_includes_required_keys():
    for artifact in (
        ...
        out = summarize_tie_order_share(artifact)
...
def test_headline_happy_path_exact_format():
    ...
    assert tie_order_share_headline(summary) == "tie-order share: 50.0% (2/4 categorized task(s))"
...
def test_headline_none_share_shows_na():
    assert tie_order_share_headline({"total": 3, "tie": 1, "tie_order_share": None}) == (
        "tie-order share: n/a (1/3 categorized task(s))"
    )


def test_headline_nan_share_shows_na():
    out = {"total": 3, "tie": 1, "tie_order_share": float("nan")}
    assert tie_order_share_headline(out) == "tie-order share: n/a (1/3 categorized task(s))"
...
def test_summarize_does_not_mutate_artifact():
    art = _stats(agree=4, disagree=0, tie=1, single=0, offline=0)
