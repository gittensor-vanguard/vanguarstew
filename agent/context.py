...
def _context_from_git(repo_path: str) -> dict:
    ...
    # Record the committer ISO date alongside the sha and subject. `recent_commits` is
    # serialized verbatim into the agent prompt, so omitting the date here made the agent see
    # per-commit dates on the frozen path but not on this fallback -- the same repo, a
    # different prompt, purely by code path. Mirrors build_context's %H%x09%cI%x09%s (#1307).
    ...
    for line in log.splitlines():
        ...
        if len(parts) == 3:
            commits.append(
                {"sha": parts[0][:10], "date": parts[1], "subject": _mask_forward_refs(parts[2])}
            )
