...
def load_context(repo_path: str) -> dict:
    ...
    if os.path.exists(path):
        try:
            ...
            #   - ValueError: malformed JSON. json.JSONDecodeError (truncated/partial write from
            #     an interrupted freeze) subclasses ValueError, so the invalid-JSON behavior is
            #     unchanged; the broader catch also covers a bare ValueError from an oversized
            #     integer literal (>4300 digits), which json.load raises directly since Python 3.11
            #     instead of a JSONDecodeError, mirroring agent/llm.py::chat_json.
