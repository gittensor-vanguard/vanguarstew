...
def context_for_agent(context: dict) -> dict:
    ...
    flag is false or missing we omit ``labels`` from the agent-facing prompt view, so ``[]``
    is not misread as "this item had no labels at T" when the real meaning is "label history
