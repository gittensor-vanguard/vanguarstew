...
_GH_LINK = re.compile(
    # The scheme is optional, mirroring benchmark/leakage.py: GitHub and markdown auto-link a
    # scheme-less `github.com/...`, so a deep-link written without `https://` (common in commit
    # subjects, issue titles, READMEs) is just as much a forward-reference and must be masked.
    # The `(?<![\w.])` boundary keeps a look-alike host (`notgithub.com`, `foo.github.com`) out.
