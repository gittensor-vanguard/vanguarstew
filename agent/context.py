...
def _context_from_git(repo_path: str) -> dict:
    ...
    # Match benchmark/freeze.py::build_context: sha + committer date + subject so planner
    # release-timing paths (_last_release_dt / _days_since_last_release) see real dates (#2253).
    ...
    return {
        "frozen_at": {
            "commit": head[:10],
            "date": commits[0]["date"] if commits else None,
        },
