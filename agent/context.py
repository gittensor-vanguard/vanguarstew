...
def _context_from_git(repo_path: str) -> dict:
    ...
    # Same ``git log`` format and parse as ``benchmark/freeze.py::build_context`` so both
    # builders emit ``{sha, date, subject}``. Dropping ``date`` here made the agent see
    # per-commit dates on the frozen path but not on the fallback path (#1275 / #1307).
    # Subject is still masked inline; ``build_context`` relies on ``scrub_context`` for the
    # same result (guarded by the builder-parity tests).
