...
1. ``run_completed`` - the run produced a scored result: no top-level ``error``, no per-repo
...
def _per_repo_error(result: dict):
    """The first per-repo error in a ``run_multi_replay`` artifact's ``per_repo`` list, if any.

    A repo that fails to clone or freeze does not abort the batch: ``run_multi_replay`` records
    it inline as ``per_repo[i] == {"error": ..., "tasks": 0}`` and keeps going, so the artifact
    never gets a top-level ``error`` for it. ``run_completed`` has to scan ``per_repo`` too, or a
    batch with a failed repo can still promote on the strength of the repos that did succeed.
    """
...
def check_promotion(result, min_composite: float = DEFAULT_MIN_COMPOSITE,
    ...
    *genuinely* scored run whose composite happens to be ``0.0`` (``scored_repos > 0``, or a
    ...
    repo_error = _per_repo_error(result)
    completed = not top_error and not repo_error and composite is not None
