"""Report the tie outcome share from a replay artifact's judge order stats.

``order_agree_rate`` reports agree rate among dual-order tasks; this read-only utility reports
how many categorized judge outcomes ended in a tie (``tie / total`` in ``judge_order_stats``),
...
def _slice_summary(slice_) -> dict:
    """``total``/``tie``/``tie_order_share`` for one replay slice."""
    ...
    tie = counts[_TIE_IDX]
    if total == 0:
        return {"total": 0, "tie": tie, "tie_order_share": None}
    return {
        "total": total,
        "tie": tie,
        "tie_order_share": round(tie / total, 3),
    }


def summarize_tie_order_share(artifact) -> dict:
    """Return tie outcome share for a replay ``artifact``."""
    ...
    if kind == "generalization":
        ...
        ties = [tuned.get("tie"), held.get("tie")]
        if all(_is_int(value) for value in totals) and all(_is_int(value) for value in ties):
            total = sum(totals)
            tie = sum(ties)
            overall = {
                "total": total,
                "tie": tie,
                "tie_order_share": round(tie / total, 3) if total > 0 else None,
            }
        else:
            overall = {"total": None, "tie": None, "tie_order_share": None}
        return {
            "kind": kind,
            **overall,
...
def tie_order_share_headline(summary: dict) -> str:
    """A one-line human summary of a :func:`summarize_tie_order_share` result."""
    summary = _dict(summary)
    total = summary.get("total")
    if not _is_int(total) or total == 0:
        return "tie-order share: no judge stats available"
    share = summary.get("tie_order_share")
    share_txt = f"{share:.1%}" if _is_number(share) else "n/a"
    tie = summary.get("tie")
    tie_txt = str(tie) if _is_int(tie) else "n/a"
    return f"tie-order share: {share_txt} ({tie_txt}/{total} categorized task(s))"
