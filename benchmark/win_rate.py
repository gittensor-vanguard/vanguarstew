...
``run_eval`` records a ``tally`` of pairwise outcomes, but nothing exposes the normalized
win/loss/tie rates for trending or CI dashboards. ``summarize_win_rate`` turns the tally into
stable percentages alongside the raw counts.
...
def _tally_counts(result: dict) -> tuple[int, int, int] | None:
    """Return ``(challenger, baseline, tie)`` when the tally is complete, else ``None``."""
...
def win_rate_headline(summary: dict) -> str:
    ...
    cr = summary.get("challenger_rate")
    cr_txt = f"{cr:.1%}" if isinstance(cr, (int, float)) and not isinstance(cr, bool) else "n/a"
    return (
        f"win rate: challenger {summary.get('challenger')}/{total} ({cr_txt}), "
