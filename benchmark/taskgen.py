...
def revealed_window(repo: str, commits: list, idx: int, n: int) -> list:
    ...
    for sha in commits[idx + 1: idx + 1 + n]:
        ...
        # --first-parent so a merge reports the files it brought to the main line;
        # git show's default combined diff would otherwise leave clean merges empty.
        # -z emits NUL-delimited paths so filenames with spaces or shell-sensitive
        # characters survive intact (whitespace .split() would corrupt them).
        out = _git(repo, "show", "--first-parent", "--name-only", "-z", "--pretty=format:", sha,
                   check=False)
