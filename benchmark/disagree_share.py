...
``disagreement_outlook`` reads ``disagreement_rate`` from judge telemetry; this read-only utility
reports the underlying ``disagree / total`` share from ``judge_order_stats``, with per-partition
detail for a ``--generalization`` artifact.
...
def _slice_summary(slice_) -> dict:
    ...
    return {
        "total": total,
        "disagree": disagree,
        "disagree_share": round(disagree / total, 3),
    }


def summarize_disagree_share(artifact) -> dict:
    """Return position-disagreement share for a replay ``artifact``."""
