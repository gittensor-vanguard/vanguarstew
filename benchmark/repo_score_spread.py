...
def _is_unscored_repo(entry: dict) -> bool:
    ...
    Distinct from :func:`_is_unscored_slice`, which keys on ``scored_repos`` — that field marks
    an unscored *aggregate* (a failed partition or a whole run), while a per-repo row carries a
    ``tasks`` count and no ``scored_repos``. Both guard the loop because the two describe
    different shapes: neither subsumes the other on a real ``per_repo`` entry.
    """
    ...
    if isinstance(per_repo, list):
        ...
        for entry in per_repo:
            if (
                ...
                and not _is_unscored_repo(entry)
