...
def run_multi_replay(repos, **kwargs) -> dict:
    ...
    Each per-repo `composite_mean` comes from `run_replay` (the composite-score anchor); only
    results that actually carry it are aggregated, so an errored/short repo can never raise.
