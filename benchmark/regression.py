...
have **completed clean** (no run-level ``error`` and no ``per_repo`` row that failed to
clone/freeze, on the compared headline partition), the candidate must not drop the headline
composite by more than ``max_composite_drop``, and must not make the pairwise judge materially
less stable (order-``disagreement_rate`` rising by more than ``max_disagreement_increase``).
Disagreement rates are recomputed from ``judge_order_stats`` when available, falling back to
``judge_report`` only when stats are absent — mirroring ``check_judge``.
...
def _headline_source(artifact: dict) -> dict:
    ...
    A ``--generalization`` artifact (both ``tuned`` and ``held_out`` present as dicts) scores on
    its **tuned** partition; everything else — including an artifact carrying a lone ``tuned``
    block with ``held_out`` absent or non-dict, which ``headline_score`` scores at the top level —
    is evaluated at the top level. Requiring *both* partitions keeps the error scan pointed at
    exactly the source ``both_scored`` compares. Mirrors ``benchmark.trend.headline_score`` and
    ``check_promotion``'s ``_promotion_source``.
    """
...
def _run_error(artifact):
    """The first error the compared run carries, or ``None`` when it completed clean.

    Checks the artifact's top-level ``error`` first (a whole-run failure is surfaced by name,
    never masked by per-repo rows), then scans the headline partition — the exact source whose
    composite feeds ``both_scored`` — with :func:`benchmark.acceptance._partition_error`, the
    canonical detector ``check_acceptance`` (#1056), ``check_promotion`` (#1254), and
    ``sample_adequacy`` already share. That helper flags a whole-partition ``error``, any
    ``per_repo[i]`` row with a truthy ``error`` (a repo that failed to clone/freeze does not abort
    the batch; ``run_multi_replay`` records it as ``{"error": ..., "tasks": 0}`` and counts it in
    ``skipped``), and a malformed row that is itself a non-empty error string — while tolerating a
    non-dict partition, a missing/non-list ``per_repo``, falsy ``error`` values, and
    non-dict/non-string rows without raising.
    """
    artifact = _dict(artifact)
...
def check_regression(candidate, baseline,
    ...
    completed clean on the compared (headline) partition — no run-level ``error`` and no
    ...
    whether it is the candidate being gated or the baseline it is gated against.
    """
    ...
    # dirty baseline makes the comparison itself meaningless. Mirrors check_acceptance (#1056)
    # and check_promotion.run_completed (#1254). A failed held_out partition stays intentionally
    # ignored — only the compared headline partition is scanned.
    base_error = _run_error(baseline)
    cand_error = _run_error(candidate)
    ...
    both_clean = base_error is None and cand_error is None
    ...
    if both_scored:
        ...
        scored_detail = (f"a run did not complete clean: baseline error {base_error!r}, "
                         f"candidate error {cand_error!r}")
    else:
