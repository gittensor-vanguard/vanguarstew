...
``order_agree_rate`` reports the *agree* rate among dual-order tasks only, and
``disagreement_outlook`` gives a qualitative read on judge instability; this read-only utility
reports how many *categorized* judge outcomes ended in a position disagreement
(``disagree / total`` in ``judge_order_stats``, where ``total`` is every categorized outcome —
``agree + disagree + tie + single + offline``), with per-partition detail for a
``--generalization`` artifact.

It is the position-bias counterpart of ``agree_order_share`` and completes the judge-order stats
share family alongside ``agree_order_share``, ``tie_order_share``, ``single_order_share``,
``dual_order_share``, and ``offline_share``. Position-bias dashboards want the disagree share as a
fraction of *all* categorized outcomes, not just among dual-order tasks (which is what
``judge_order_stats.disagreement_rate`` already reports).
