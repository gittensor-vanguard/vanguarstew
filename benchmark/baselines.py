...
def _safe_backlog(context: dict, key: str) -> list:
    ...
    structurally denied. A non-list value is treated as no entries (#515)."""
    if isinstance(context, dict) and context.get("_issues_truncated") is True:
        return []
    return _baseline_list((context or {}).get(key), key)
