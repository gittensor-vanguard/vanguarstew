...
contract at ``https://polaris.computer/docs``.  Its v1 quote binds a fresh caller challenge and the
...
def expected_report_data(
    nonce_hex: str, e2e_pubkey_b64: str, image_digest: str, stdout: str
) -> bytes:
    """Compute the 64-byte binding used by Polaris's ``POST /v1/attest`` contract."""
    ...
    result = hashlib.sha256((image_digest + stdout_hex).encode("utf-8")).digest()
...
class PolarisReceipt:
    ...
    def from_response(cls, response) -> "PolarisReceipt":
        ...
        binding_version = raw_version if isinstance(raw_version, int) else 0
        return cls(
            quote_b64=quote_b64,
    ...
    def from_dict(cls, value) -> "PolarisReceipt":
        ...
        return cls(
            ...
            binding_version=(
                value.get("binding_version") if isinstance(value.get("binding_version"), int) else 1
            ),
        )
...
def verify_receipt(
    ...
    *,
    ...
    try:
        ...
        expected = expected_report_data(nonce, e2e_pubkey_b64, parsed.image_digest, parsed.stdout)
    ...
    checks = {
        ...
        # Vanguarstew currently implements the no-files/no-recording v1 formula.  Polaris v2 folds
        # mounted-file and egress-log hashes into the high half and needs a separate verifier path.
        "binding_version": parsed.binding_version == 1,
    }
