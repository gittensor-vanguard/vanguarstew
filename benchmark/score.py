...
def _plan_tokens(plan) -> set:
    """Content tokens from a plan for module/backlog recall: item ``title``, ``theme``, and
    the path segments of any structured ``files`` (plus plain-string items verbatim).
    ...
    naming it — farming the deterministic anchor. Commit-kind anticipation is scored
    separately by ``kind_recall``, which reads ``kind`` directly. Real path segments in
    ``files`` genuinely name the module, so they stay.
    """
    toks = set()
    for item in plan or []:
