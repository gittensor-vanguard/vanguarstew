...
def _plan_content_tokens(plan) -> set:
    """Tokens from a plan item's title, theme, and structured ``files`` — the content used
    for module and backlog recall.
    ...
    for item in plan or []:
        if isinstance(item, dict):
            ...
            # top-level module even when the title omits it. Real path segments legitimately
            # name the module, unlike a bare `kind` tag, so they don't farm recall.
