...
regression, and leaderboard ranking, but nothing verifies they actually agree. A hand-edited or
mis-assembled artifact could report a flattering ``decisive_margin`` while the per-task rows tell
a different story.
...
2. ``tally_sums_to_tasks`` — the three counts sum to ``tasks``;
3. ``rows_match_tasks`` — when per-task ``rows`` are present, ``len(rows) == tasks``;
4. ``row_winners_match_tally`` — winner labels in ``rows`` recount to the same ``tally``;
5. ``decisive_margin_matches`` — ``decisive_margin`` equals ``challenger - baseline`` when present.

Multi-repo and ``--generalization`` artifacts are checked per scored repo entry (or per partition).
...
def _count_row_winners(rows: list) -> dict:
...
def _check_slice(label: str, slice_: dict, checks: list) -> None:
    ...
    if tally is not None and tasks_ok:
        total = sum(tally[k] for k in _TALLY_KEYS)
        add("tally_sums_to_tasks", total == int(tasks),
            f"tally sum {total} == tasks {int(tasks)}")
    else:
        add("tally_sums_to_tasks", False, "cannot compare tally to tasks")

    rows_key_present = "rows" in slice_
    rows = _rows_list(slice_.get("rows")) if rows_key_present else None
    if rows_key_present:
        add("rows_match_tasks", tasks_ok and rows is not None and len(rows) == int(tasks),
            f"{len(rows or [])} row(s) for {int(tasks) if tasks_ok else tasks} task(s)")
        if tally is not None and rows:
            row_counts = _count_row_winners(rows)
            match = all(row_counts[k] == tally[k] for k in _TALLY_KEYS)
            add("row_winners_match_tally", match,
                f"row winners {row_counts} vs tally {tally}")
        elif rows_key_present:
            add("row_winners_match_tally", False, "cannot recount row winners")
